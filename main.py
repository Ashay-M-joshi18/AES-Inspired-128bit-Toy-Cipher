...
def encryption(hex_output):
    ...
    return deb_bck(state, hex_output)


# ---------- HEX → MATRIX (FOR DECRYPTION) ----------
...
def decryption():
    hex_input = input("Enter hex ciphertext: ")
    state = hex_to_matrix(hex_input)
    ...
    return deb_bck(state, False)


# ---------- RUN ----------

def run():
    print("Instruction: Enter 1 for encryption and Enter 2 for decryption:\n")
    Select = int(input("Enter your choice: "))

    if Select == 1:
        choice = int(input("Enter 1 for hex , 2 for string: "))
        hex_output = True if choice == 1 else False
        cipher = encryption(hex_output)
